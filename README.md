...
### Customization
After setting up, the app can be customized very easily. Currently, the app is designed so that only the products "Microsoft Teams", "Bookings" and "SharePoint" can be displayed and filtered. If other topics are more relevant to you, this can be changed in the individual galleries. In the SharePoint list you can alse decide which posts are displayed. For irrelevant posts simply remove them from the SharePoint list.   
